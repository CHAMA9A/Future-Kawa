...
## Perspective d'intégration ERP / système d'information

| Critère | Description |
|---|---|
| **Positionnement** | FutureKawa est une brique métier spécialisée (suivi des lots, IoT, alertes qualité) et non un ERP complet |
| **Point d'accès unique** | Le backend central (`/api/central/*`) offre un point d'entrée REST unique pour les trois pays |
| **Documentation API** | Tous les endpoints sont documentés via Swagger sur chaque service (`/api/docs`) |
| **Format standard** | Les données sont échangées en JSON, format universellement supporté par les ERP et progiciels |

FutureKawa expose des API REST documentées qui permettent à un ERP externe (SAP, Odoo, Microsoft Dynamics) de consulter les lots, les mesures IoT, les alertes qualité et les seuils configurés. Les endpoints FIFO facilitent l'intégration avec des systèmes logistiques. L'architecture API-first permet un raccordement sans modification du code applicatif. À ce stade, l'intégération repose sur les API REST brutes — aucun connecteur ERP prêt à l'emploi n'est encore développé, et les endpoints ne sont pas protégés par authentification.

Documentation complète : [docs/erp-integration.md](erp-integration.md)
