 ...
 * - Écoute sur le port défini dans .env (défaut: 3012)
 */
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('FutureKawa Colombia Service API')
    .setDescription('API documentation for FutureKawa Colombia Service')
    .setVersion('1.0')
    .build();
  ...
  console.log(`🚀 Colombia Service démarré sur http://localhost:${port}`);
